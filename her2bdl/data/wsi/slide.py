...
#
# WSI hamatsu constants
# tiff properties
WSI_PROPERTIES_DATETIME = "tiff.DateTime"
WSI_PROPERTIES_MAKE = "tiff.Make"
WSI_PROPERTIES_MODEL = "tiff.Model"
WSI_PROPERTIES_SOFTWARE = "tiff.Software"

# hamatsu properties
WSI_PROPERTIES_WHITE_INTENSITY = "hamamatsu.target.white.intensity"
...
def describe_slide(slide, to_file=None):
    """
    Display description of slide
    ...
    to_file : `str` or `None`
        if is not None, write description to file instead of printing.
    Returns:
      `None`
    """
    # Image attributes
    ...
    # tiff properties
    datetime = slide.properties.get(WSI_PROPERTIES_DATETIME)
    make     = slide.properties.get(WSI_PROPERTIES_MAKE)
    model    = slide.properties.get(WSI_PROPERTIES_MODEL)
    software = slide.properties.get(WSI_PROPERTIES_SOFTWARE)
    # hamatsu properties
    intensity = slide.properties.get(WSI_PROPERTIES_WHITE_INTENSITY)
    # write description
    description = f"{file}\nmodel: {make} {model} {software}\n"
    description += f"vendor: {vendor}\n"
    description += f"datetime: {datetime}\ndimensions: {dimensions}\nlevels: {levels}\n"
    for i, (level_dim, level_down) in enumerate(zip(level_dimensions, level_downsamples)):
        description += f"\t[{i}]: dimensions={level_dim}\tdownsample={level_down}\n"
    # 
